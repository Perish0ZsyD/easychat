...
public class GroupInfoServiceImpl implements GroupInfoService{
	...
	public void saveGroup(GroupInfo groupInfo, MultipartFile avatarFile, MultipartFile avatarCover) {
		...
		if (StringTools.isEmpty(groupInfo.getGroupId())) {
			...
			groupInfo.setCreateTime(curDate); // getGroupId和getUserId方便分辨
			...
			userContact.setCreateTime(curDate); // 当前时间
			userContact.setLastUpdateTime(curDate); // 最后更新时间
...
//			messageHandler.sendMessage(messageSend);
			...
			// 上面判断逻辑：如果张三指导李四的群组id是不是可以修改李四群组，上面就是判断当前群组id的所有者是不是前面传过来的（防止绕过前端走接口恶意修改）
